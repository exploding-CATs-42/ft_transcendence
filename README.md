...
A single command bootstraps the entire project — it creates the `.env` files from their
templates, builds and starts every container, installs dependencies inside them, applies
the database migrations, and seeds demo data:

```bash
make setup
```

When it finishes it prints the URL to open. Because the TLS certificate is self-signed,
your browser will show a warning on first visit — accept it to continue.

### Configuration

`make setup` copies each `.env.example` to its `.env` if one does not already exist. The
defaults in the templates are placeholders: fill them in before running the stack in any
shared environment.
...
`make setup` is a wrapper. If you prefer to run each stage yourself, or need to debug one
of them, the individual targets still exist and work exactly as before:
...
make build          # build images and start containers, then follow backend logs
make deps           # install pnpm dependencies inside the containers
make prisma-deploy  # apply existing database migrations
make seed           # insert demo users and card reference data
```

`make deps` is required because `node_modules` are mounted as separate Docker volumes, so
the install has to happen inside the running containers rather than at image build time.
...
After changing a Dockerfile or image-level dependency, re-run `make build && make deps`.
After changing the Prisma schema, run `make prisma-migrate name=your_migration_name`.
...
| Category | Targets |
| --- | --- |
| Lifecycle | `setup`, `up`, `build`, `up-build`, `down`, `re`, `ps`, `clean` |
| Logs | `logs`, `logs-backend`, `logs-frontend`, `logs-nginx`, `logs-postgres` |
| Shells | `backend-shell`, `frontend-shell`, `db-shell` |
| Database | `prisma-generate`, `prisma-migrate name=…`, `prisma-deploy`, `prisma-reset`, `seed` |
...
| `Module not found` in a container | Run `make deps` — dependencies live in container volumes |
...
| Profile editing | Change username, email, or password; changing a password requires the current one | Valeriia (frontend), Ivan (backend) |
...
| Friends list | Filterable by accepted, incoming, or all | Valeriia (frontend), Ivan (backend) |
| User search | Look up players by exact username | Valeriia (frontend), Ivan (backend) |
...
| Feature | Description | Contributors |
| --- | --- | --- |
| Lobby page | Browse open tables with player slots and avatars | Roman (created), Ivan (feature work) |
...
| Feature | Description | Contributors |
| --- | --- | --- |
| Game rules engine | The complete rule set as an XState machine with 19 states, shared by frontend and backend | Roman, Andrii, Valeriia |
...
**How:** 21 application endpoints across five resource groups (`/auth`, `/users`, `/me`,
...
35 broadcast server events, 12 private events, and 15 typed error events, all defined once
...
win condition — the last player alive wins. The rule set is an XState machine with 19
...
**How:** The application is supported on **Firefox, Edge, and Opera** in addition to
Chrome. These cover two rendering engines: **Blink** (Chrome, Edge, Opera — all built on
Chromium) and **Gecko** (Firefox). Firefox is the meaningful compatibility target, since it
shares no engine code with Chrome.

Compatibility comes from avoiding vendor-specific APIs and providing standards fallbacks:

- `modern-normalize` as a cross-browser reset
- `@supports not selector(::-webkit-scrollbar)` falls back to standard `scrollbar-color`
  for the custom scrollbar, which is the one place WebKit-prefixed CSS is used
- `height: 100vh` followed by `height: 100dvh` so browsers without dynamic viewport units
  still get a correct full-height layout
- `prefers-reduced-motion` respected for all hover and active transforms
- ES2023 output with no transpilation to legacy targets, which every supported browser
  handles natively
- Phaser renders through WebGL with an automatic Canvas fallback

**Browser-specific notes and limitations:**

- The TLS certificate is self-signed, and each browser presents a different interstitial.
  Firefox requires explicitly adding a security exception; Chromium-family browsers accept
  it through "Advanced → Proceed".
- Scrollbar styling differs by design: Chromium-family browsers get the fully custom
  scrollbar, Firefox gets the standard `scrollbar-color` treatment.
- Safari is **not** claimed. It has not been tested, and WebKit is where the WebGL and
  certificate behaviour is most likely to differ.
...
| --- | --- | --- |
| Client → server | 16 | `join-game`, `reconnect-game`, `draw-card`, `play-card`, `play-combo`, `play-nope`, `play-defuse`, `insert-kitten`, `select-player`, `give-card` |
...
| Errors | 15 | `join-game-error`, `draw-card-error`, `play-combo-error` |
...
run the test suites**. Tests are run locally. Adding a test job is on the list below.

---

## Planned Work and Known Limitations

This is an actively developed project and the items below are tracked work, not permanent
gaps.

### Planned

- Add a test job to the CI workflow so suites run on every pull request
- Remove code that is no longer wired into the application, including an unused
  experimental state machine under `frontend/src/game/entities/model/`, unused mock data,
  and the empty `packages/types` workspace
- De-duplicate the stale copy of the game rules at `backend/src/constants/rules.json`,
  which has drifted from the authoritative `packages/game/src/constants/rules.ts`
- Reconcile the Nope window duration, which is currently defined as `3000 ms` in the
  machine constants and `5000 ms` in the default rules object
- Delete replaced avatars from Cloudinary — the `removeImage` helper exists but is not yet
  called, so old images currently remain
- Add a production Docker Compose target; only a development target exists today, and the
  production stages in the Dockerfiles are not yet exercised (the backend `runner` stage
  references a `turbo.json` left over from the pre-pnpm setup)
- Remove the unused `cards` database table, or start reading card metadata from it instead
  of the shared JSON catalogue

### Out of scope by design

- **No chat.** The team chose to spend the time on game depth instead.
- **No internationalisation.** The interface is English-only.
- **Safari is not supported.** See the browser module above.
- **Usernames are not unique.** Display names may repeat; identity is the email address.
...
AI tools were used throughout the project as an assistant, never as an unreviewed author.
...
**Where AI was deliberately not relied on:**

- **Writing application code.** Features were written by hand. AI was used to understand
  unfamiliar libraries and to review and debug code the team had already written, not to
  generate the implementation.
- **Game rule design.** The state machine's structure, its states, and its transitions were
  designed by the team. Getting the Nope window right required understanding the game, not
  generating plausible code.
- **Architectural decisions.** The authoritative-server model, the choice of XState, and
  the workspace package split were team decisions made in discussion.
- **Anything not understood.** The team's rule was that no member committed code they could
  not explain in review — which is also what the evaluation requires.
